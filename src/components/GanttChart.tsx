...
export default function GanttChart() {
  ...
  // 单位转换函数
  const convertToViewUnits = (value: number, unit: 'hour' | 'day' | 'week', viewMode: 'day' | 'week' | 'month'): number => {
    // 根据当前视图模式和任务时间单位进行转换
    if (viewMode === 'day') {
      // 日视图下，所有单位转换为小时
      switch (unit) {
  ...
  // 计算任务在当前视图中的百分比位置和宽度
  const getTaskStyle = (task: Task, viewMode: 'day' | 'week' | 'month') => {
  ...
  const handleTaskClick = (task: Task, server: string) => {
    toast({
      title: "任务详情",
        ...
        <div className="flex items-center space-x-2">
          {(['day', 'week', 'month'] as const).map(mode => (
            <button 
              ...
              className={`px-3 py-1 text-sm rounded transition-colors ${viewMode === mode ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
            >
        ...
        <div className="min-w-full">
          ...
          <div className="flex border-b border-gray-200 pb-2 mb-4">
            ...
            <div className="flex-1 flex">
              {timeSlots[viewMode].labels.map((slot, index) => (
          ...
          <div className="space-y-3">
            {tasks.map((serverData, serverIndex) => (
              <div key={serverIndex} className="flex items-center">
                ...
                <div className="flex-1 h-8 bg-gray-50 rounded relative overflow-hidden">
                  {serverData.tasks.map((task, taskIndex) => (
                    <div 
                      key={taskIndex} 
                      className={`absolute h-6 ${task.color} rounded gantt-bar cursor-pointer`} 
                      style={{
                        ...getTaskStyle(task, viewMode),
                        top: '4px'
                      }} 
                      onClick={() => handleTaskClick(task, serverData.server)} 
