...
export default function Sidebar({
  ...
  return <aside className="w-64 bg-white shadow-lg overflow-y-auto">
      <div className="p-4">
        {/* 用户信息卡片 */}
        <div className="bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg p-4 text-white mb-4">
          <div className="flex items-center space-x-3">
            <img className="h-12 w-12 rounded-full border-2 border-white" src={user?.avatar || 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=48&h=48&fit=crop&crop=face'} alt="用户头像" />
            <div>
        ...
        <div className="space-y-3">
          <button className="w-full flex items-center justify-between p-3 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors">
            ...
            {notifications > 0 && <span className="bg-blue-600 text-white text-xs px-2 py-1 rounded-full">{notifications}</span>}
          </button>
          ...
          <button className="w-full flex items-center justify-between p-3 bg-green-50 text-green-700 rounded-lg hover:bg-green-100 transition-colors">
            ...
            {messages > 0 && <span className="bg-green-600 text-white text-xs px-2 py-1 rounded-full">{messages}</span>}
          </button>
          ...
          <button className="w-full flex items-center justify-between p-3 bg-orange-50 text-orange-700 rounded-lg hover:bg-orange-100 transition-colors">
            ...
            {reminders > 0 && <span className="bg-orange-600 text-white text-xs px-2 py-1 rounded-full">{reminders}</span>}
          </button>
        </div>
      </div>
    </aside>;
