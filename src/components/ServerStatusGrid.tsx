...
interface Server {
  id: number
  name: string
  status: 'running' | 'idle' | 'full' | 'error'
  cpu: number | null
  memory: number | null
  statusText: string
  statusColor: string
...
export default function ServerStatusGrid() {
  ...
  const [draggedItem, setDraggedItem] = useState<Server | null>(null)
  const [dragOverItem, setDragOverItem] = useState<Server | null>(null)
  ...
  // 处理拖拽经过
  const handleDragOver = (e: React.DragEvent, server: Server) => {
    e.preventDefault()
    // 只允许相同类型和状态组的服务器之间拖拽
    if (draggedItem && 
  ...
  // 切换展开/收起状态
  const toggleExpanded = (section: 'internalNormal' | 'internalError' | 'rentalNormal' | 'rentalError') => {
  ...
  // 获取服务器项目的样式类
  const getItemClass = (server: Server) => {
  ...
  // 过滤服务器数据
  const getVisibleServers = (type: 'internal' | 'rental', isError: boolean, limit: number = 2) => {
    const filtered = servers.filter(server => 
      server.type === type && (server.status === 'error') === isError
    )
    
    // 根据展开状态决定显示数量
    const sectionKey = `${type}${isError ? 'Error' : 'Normal'}` as keyof typeof expandedSections
    return expandedSections[sectionKey] ? filtered : filtered.slice(0, limit)
  }

  // 获取隐藏的服务器数量
  const getHiddenServerCount = (type: 'internal' | 'rental', isError: boolean) => {
